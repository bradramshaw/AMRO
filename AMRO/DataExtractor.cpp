...
DataExtractor::DataExtractor(std::string name) // constructor
{
	...
	dataArray = ArrayAllocator::AllocateDynamicArray<double>(number_of_lines); //allocates an array of the appropriate size, right now 3 wide MAY NEED ADJUSTMENT
	bool y = dataConverter(&buffer1, dataArray);
}

DataExtractor::~DataExtractor() // destructor 
{
	ArrayAllocator::FreeDynamicArray<double>(dataArray);
}
